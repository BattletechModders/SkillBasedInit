...
namespace SkillBasedInit.Helper {
    public class UnitHelper {
        ...
        private static int GetTonnageRange(float tonnage) {
            ...
            Mod.Log.LogIfDebug($"for raw tonnage {tonnage} returning tonnageRange:{tonnageRange}");
            return tonnageRange;
        }
            ...
            if (actor.StatCollection != null && actor.StatCollection.ContainsStatistic("BaseInitiative")) {
                ...
                switch (weightClass) {
                    ...
                    case WeightClass.ASSAULT:
                        ...
                        break;
                    default:
                        Mod.Log.LogIfDebug($"Actor:{actor.DisplayName}_{actor.GetPilot().Name}" +
                ...
                unitInit = baseMod * -1;
                Mod.Log.LogIfDebug($"Normalized BaseInit for Actor:{actor.DisplayName}_{actor.GetPilot().Name}" +
        ...
        public static int GetNormalizedComponentModifier(MechDef mechDef) {
            ...
            Mod.Log.LogIfDebug($"Normalized BaseInit for mechDef:{mechDef.Name} is unitInit:{unitInit}");
            return unitInit;
        }
        ...
        public static int GetEngineModifier(AbstractActor actor) {
            ...
            if (actor.GetTags().Contains("unit_powerarmor")) {
                Mod.Log.LogIfDebug($"  Actor:{actor.DisplayName}_{actor.GetPilot().Name} is PowerArmor, skipping engine bonus.");
            } else {
                ...
                if (mainEngineComponent != null) {
                    ...
                    Mod.Log.LogIfDebug($"  Actor:{actor.DisplayName}_{actor.GetPilot().Name} with engine rating: {engine?.Rating} has engineMod:{engineMod}");
                } else {
                    Mod.Log.Log($"  Actor:{actor.DisplayName}_{actor.GetPilot().Name} has no engine - is this expected?");
                }
            }
        ...
        public static int GetEngineModifier(MechDef mechDef) {
            ...
            Mod.Log.LogIfDebug($"MechDef:{mechDef.Name} has engineComponent:{engineRef}?");
            if (engineRef != null) {
        ...
        private static int CalculateEngineModifier(float tonnage, int rating) {
            ...
            Mod.Log.LogIfDebug($"Comparing engineRatio:{engineRatio} from rating:{rating} / tonnage:{tonnage} vs midpoint:{ratioMidpoint}");
            if (engineRatio > ratioMidpoint) {
                ...
                Mod.Log.LogIfDebug($"Oversized engine, returning bonus engineMod:{engineMod}");
                ...
                Mod.Log.LogIfDebug($"Undersized engine, returning penalty engineMod:{engineMod}");
            } else {
                Mod.Log.LogIfDebug("Balanced engine, returning engineMod:0");
                engineMod = 0;
            }
