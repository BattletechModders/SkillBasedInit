    ...
    public class ActorInitiative {
        ...
        */
        ...
        public ActorInitiative(AbstractActor actor) {
            ...
            Mod.Log.LogIfDebug($"Actor:{actor.DisplayName}_{pilot.Name} has meleeAttackMod:{meleeAttackMod} meleeDefenseMod:{meleeDefenseMod}");
            ...
            Mod.Log.Log($"Actor:{actor.DisplayName}_{pilot.Name} has " +
        ...
        public void CalculateRoundInit(AbstractActor actor) {
            ...
            if (actor.IsDead || actor.IsFlaggedForDeath) {
                ...
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) is dead, skipping init.");
                return;
            }
            ...
            Mod.Log.LogIfDebug($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has roundInit:{roundInitiative} = (roundInitbase:{roundInitBase} - roundVariance:{roundVariance})");
            ...
            if (actor.IsMoraleInspired || actor.IsFuryInspired) {
                ...
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) is inspired, added {bonus} = roundInit:{roundInitiative}");                
            }
            ...
            if (deferredInjuryMod != 0) {
                ...
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) was injured on a previous round! Subtracted {deferredInjuryMod} = roundInit:{roundInitiative}");                
                ...
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has existing injuries! Subtracted {penalty} = roundInit:{roundInitiative}");                
            }
            ...
            if (isMovementCrippled) {
                ...
                Mod.Log.LogIfDebug($"  Crippled Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has rawMod:{rawMod} = ({Mod.Config.CrippledMovementModifier} - {this.pilotingEffectMod})");

                int penalty = Math.Min(-1, rawMod);
                roundInitiative += penalty;
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has crippled movement! Subtracted {penalty} = roundInit:{roundInitiative}");                
            }

            // Check for prone 
            if (actor.IsProne) {
                int rawMod = Mod.Config.ProneModifier + this.pilotingEffectMod;
                Mod.Log.LogIfDebug($"  Prone Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has rawMod:{rawMod} = ({Mod.Config.ProneModifier} - {this.pilotingEffectMod})");

                int penalty = Math.Min(0, rawMod);
                roundInitiative += penalty;
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) is prone! Subtracted {penalty} = roundInit:{roundInitiative}");                
            }

            // Check for shutdown
            if (actor.IsShutDown) {
                int rawMod = Mod.Config.ShutdownModifier + this.pilotingEffectMod;
                Mod.Log.LogIfDebug($"  Shutdown Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has rawMod:{rawMod} = ({Mod.Config.ShutdownModifier} - {this.pilotingEffectMod})");

                int penalty = Math.Min(0, rawMod);
                roundInitiative += penalty;
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) is shutdown! Subtracted {penalty} = roundInit:{roundInitiative}");                
            }

            // Check for melee impacts        
            if (this.deferredMeleeMod > 0) {
                roundInitiative -= deferredMeleeMod;
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) was meleed after activation! Subtracted {this.deferredMeleeMod} = roundInit:{roundInitiative}");
            }
            ...
            if (this.reservedCount > 0) {
                ...
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) deferred last round! " +
            ...
            if (this.deferredCalledShotMod > 0) {
                ...
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) was targetd by called shot after activation! Subtracted {this.deferredCalledShotMod} = roundInit:{roundInitiative}");                
            }

            // Check for deferred vigilance bonus
            if (this.deferredVigilanceMod > 0) {
                roundInitiative += deferredVigilanceMod;
                Mod.Log.Log($"  Actor:({actor.DisplayName}_{actor.GetPilot().Name}) did vigilance last round! Adding {this.deferredVigilanceMod} = roundInit:{roundInitiative}");
            }

            if (roundInitiative <= 0) {
                roundInitiative = Mod.MinPhase;
                Mod.Log.LogIfDebug($"  Round init {roundInitiative} less than 0, setting to 1.");
            } else if (roundInitiative > 30) {
                roundInitiative = Mod.MaxPhase;
                Mod.Log.LogIfDebug($"  Round init {roundInitiative} greater than 30, setting to 30.");
            }

            // Init is flipped... 1 acts in first phase, then 2, etc.
            actor.Initiative = (Mod.MaxPhase + 1) - roundInitiative;
            Mod.Log.Log($"== Actor:({actor.DisplayName}_{actor.GetPilot().Name}) has init:({roundInitiative}) from base:{roundInitBase} - variance:{roundVariance} plus modifiers.");
        }
        ...
        public static int CalculateMeleeDelta(ActorInitiative attacker, ActorInitiative target) {
            ...
            Mod.Log.LogIfDebug($"Melee rawDelta:{rawDelta} = (attackerMod:{attacker.meleeAttackMod} - targetMod:{target.meleeDefenseMod})");
            int modifiedDelta = Math.Max(1, rawDelta - target.gutsEffectMod);
            Mod.Log.LogIfDebug($"MeleeMod reduced to modifiedDelta:{modifiedDelta} = (rawDelta:{rawDelta} - gutsEffectMod:{target.gutsEffectMod})");
        ...
        public void ResolveMeleeImpact(AbstractActor target, int impact) {
            ...
            if (target.HasActivatedThisRound) {
                Mod.Log.Log($"Melee impact will slow Actor:({target.DisplayName}_{target.GetPilot().Name}) by {impact} init on next activation!");
                ...
                Mod.Log.Log($"Melee impact immediately slows Actor:({target.DisplayName}_{target.GetPilot().Name}) by {impact} init!");
        ...
        public int CalculateInjuryPenalty(int damageTaken, int existingInjuries) {
            ...
            Mod.Log.LogIfDebug($"  InjuryPenalty:-{injuryPenalty} from (injuryMin:{this.injuryBounds[0]} + existingInjuries:{existingInjuries}) to (injuryMax:{this.injuryBounds[1]} + existingInjuries:{existingInjuries} + damageTaken:{damageTaken}).");
